...
// Supabase's newer "secret" key (sb_secret_…) OR the legacy service_role key.
// Either works for server-side REST calls; both bypass Row-Level Security.
const SERVICE_KEY =
  process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
const useSupabase = !!(SUPABASE_URL && SERVICE_KEY);

// Each submission is auto-deleted 24h after it ended, one at a time — not a
// bulk sweep on a timer, but pruned lazily whenever the list is read (the
// teacher panel polls every 5s while open, so in practice this is near-live).
// Keep this in sync with the matching constant in teacher-app.js, which uses
// it to render the per-row countdown.
...
// Sanitise an incoming suggestion/problem report (never trust the client).
...
const supabaseStore = {
  ...
  async listSubmissions() {
    const cutoff = new Date(Date.now() - SUBMISSION_TTL_MS).toISOString();
    await sb('submissions?ended_at=lt.' + encodeURIComponent(cutoff), { method: 'DELETE' });
  ...
  async listFeedback() {
    const rows = await sb('feedback?select=*&order=id.desc');
    return (rows || []).map(mapFeedbackRow);
  },
  async addFeedback(body) {
    const rows = await sb('feedback', {
      method: 'POST',
      headers: { Prefer: 'return=representation' },
      body: JSON.stringify(cleanFeedback(body)),
    });
    return rows && rows[0] ? mapFeedbackRow(rows[0]) : { ok: true };
  },
  async clearFeedback() {
    await sb('feedback?id=gt.0', { method: 'DELETE' });
    return { ok: true };
  },
};
...
const localStore = {
  ...
  async listSubmissions() {
    ...
    const kept = rows.filter(function (r) {
      return !r.ended_at || new Date(r.ended_at).getTime() > cutoff;
    });
    if (kept.length !== rows.length) writeLocal(SUBS_FILE, kept);
    return kept.map(mapSubRow);
  },
  async addSubmission(body) {
    ensureLocal();
    const rows = readLocal(SUBS_FILE) || [];
  ...
  async listFeedback() {
    ensureLocal();
    return (readLocal(FEEDBACK_FILE) || []).map(mapFeedbackRow);
  },
  async addFeedback(body) {
    ensureLocal();
    const rows = readLocal(FEEDBACK_FILE) || [];
    const row = Object.assign({ id: ++localSeq }, cleanFeedback(body));
    rows.unshift(row);
    writeLocal(FEEDBACK_FILE, rows);
    return mapFeedbackRow(row);
  },
  async clearFeedback() { ensureLocal(); writeLocal(FEEDBACK_FILE, []); return { ok: true }; },
};
